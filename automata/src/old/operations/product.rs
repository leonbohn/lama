...
use crate::{
    ...
    ts::{
        ...
        StateReference, TransitionReference, TriggerOf,
    },
    Acceptor, Combined, Equivalent, IntoMealyTransitions, MealyMachine, Pair, Pointed, State,
    Successor, Symbol, Transformer, Transition, TransitionSystem, Trigger, Value, DBA, DFA, DPA,
};
    ...
    R: HasInput<Sigma = L::Sigma>,
{
    ...
    type Input<'me> = L::Input<'me> where Self:'me;
    ...
    R: HasStates,
{
    ...
    fn contains_state<X: Borrow<Self::Q>>(&self, state: X) -> bool {
        self.left.contains_state(state.borrow().left())
            && self.right.contains_state(state.borrow().right())
    }
}
...
pub struct ProductTransitions<L, R>
where
    L: Iterator,
{
    left: L,
...
impl<L, R> Iterator for ProductTransitions<L, R>
where
    L: Iterator,
    R: Iterator + Clone,
...
{
    type Item = (
        Pair<<L::Item as Trigger>::Q, <R::Item as Trigger>::Q>,
        <L::Item as Trigger>::S,
        Pair<<L::Item as Trigger>::Q, <R::Item as Trigger>::Q>,
    );
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if let Some(left_element) = self.left_element.clone() {
            ...
            Some((
                Pair::new(
                    left_element.source().clone(),
                    right_element.source().clone(),
                ),
                left_element.sym().clone(),
                Pair::new(
                    left_element.target().clone(),
                    right_element.target().clone(),
                ),
            ))
        } else {
...
pub fn product_transitions<L, R>(
    left_transitions: L,
    right_transitions: R,
) -> ProductTransitions<L::IntoTransitions, R::IntoTransitions>
where
    L: IntoTransitions,
...
impl<L, R> Iterator for ProductStates<L, R>
where
    L: Iterator,
    R: Iterator + Clone,
...
impl<'a, L, R> IntoStates for &'a Product<L, R>
where
    L: IntoStates,
    R: IntoStates<Sigma = L::Sigma>,
    R::IntoStates: Clone,
{
...
impl<L, R> Transformer for Product<L, R>
where
    L: Transformer,
    R: Transformer,
{
...
pub struct ProductAssignments<L, R>
where
    L: Iterator,
{
    left: L,
...
impl<'a, L, R> IntoAssignments for &'a Product<L, R>
where
    L: IntoAssignments,
    R: IntoAssignments,
    R::Assignments: Clone,
...
impl<L, R> Iterator for ProductAssignments<L, R>
where
    L: Iterator,
    R: Iterator + Clone,
...
{
    type Item = (
        Pair<<L::Item as Assignment>::Left, <R::Item as Assignment>::Left>,
        Pair<<L::Item as Assignment>::Right, <R::Item as Assignment>::Right>,
    );
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        self.left_element.as_ref().map(|a| {
            (
                Pair::new(a.left(), right_element.left()),
                Pair::new(a.right(), right_element.right()),
            )
        })
    }
}
...
mod tests {
    use crate::{
        ...
        ts::{IntoParts, IntoTransitions, Visitor},
        MealyMachine, Pair, Successor, Transformer, TransitionSystem, DFA,
    };
